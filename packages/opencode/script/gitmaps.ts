...
if (jsonOutput) {
  ...
  const [log, branches, currentBranch, ahead, behind, contributors, tags, stash] = await Promise.all([
    $`git log --oneline --graph --all -20`.text(),
    $`git branch -a`.text(),
    $`git branch --show-current`.text().then((t) => t.trim()),
    $`git rev-list --count HEAD @{u}`.text().catch(() => "0"),
    $`git rev-list --count @{u}..HEAD`.text().catch(() => "0"),
    $`git log --format="%an" | sort | uniq -c | sort -rn | head -5`.text(),
    $`git tag --sort=-creatordate | head -10`.text(),
    $`git stash list`.text(),
  ])
    ...
    JSON.stringify(
      {
        ...
        branches: branches
          ...
          .map((b) => b.trim()),
        status: status || null,
        changedFiles: diff ? diff.trim().split("\n") : [],
